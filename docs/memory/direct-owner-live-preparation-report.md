...
- direct-owner build manifest/binary identity, `memory-owner-capture` inclusion, and `snapshot-dedup` exclusion;
- existing N1 controller review identity `e707e2d` and nav pack SHA-256 `2f393138c905aaf1b2db4f77442426db01ff2dfad5ee575d77454012d27a4a30`.
...
3. The controller's current `max_wall_s` is derived as 960 s, not 360 s, and the current launcher command cannot be changed by a spec without also satisfying its controller/diagnostic timing consistency checks.
