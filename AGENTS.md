...
**Layout:** crates under `crates/{host,vault,api,host-play,panel,e2e}` (`panel` is this campaign's native UI).
...
**Do not:** invent a tick-end opcode; deep-copy the world every read; skip the brief; wander into nav/scripts/wall; rewrite the FR clone; hunt for a longer AGENTS.md.
