...
- Added the exact opt-in `--direct-owner-managed` contract and preserved the default/no-cache contract when the flag is absent.
...
`docs.memory.test_current_tui_calibration.CurrentCalibration.test_resolve_rs2b0t_commit_real_git_when_path_ok`

The container has no `git` executable. That exact real-git test passed in the macOS non-collector run.

Result: 140 tests, 0 failures, 0 errors, 2 macOS-only skips.

Log: `docker-linux-generated-unittest-rerun.log`

SHA256: `8dbe66a1b5ad108bc8bbaed8e56f6adc3553a9a114cffcfc36631bf6a6c3db3e`
...
### Preserved macOS full-suite failure

Current complete focused command result: 141 tests, 9 failures, 4 Linux-only skips.
