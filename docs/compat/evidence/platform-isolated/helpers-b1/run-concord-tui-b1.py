...
os.close(master);code=p.wait();raw=(out/'terminal.log').read_bytes();decoded=raw.decode(errors='replace');bad=('panicked at' in decoded or 'FAIL:' in decoded or 'not impl:' in decoded);passed='PASS:' in decoded;r.update(finished_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),elapsed_seconds=round(time.monotonic()-before,3),process_exit_code=code,wall_timeout=timeout,runtime_error=bad,pass_marker=passed,exit_code=code or int(timeout or bad or not passed),terminal_sha256=hashlib.sha256(raw).hexdigest(),terminal_bytes=len(raw),input_forwarded_count=control_seen,runner_sha256=hashlib.sha256(pathlib.Path(__file__).read_bytes()).hexdigest());(out/'receipt.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r),flush=True);sys.exit(r['exit_code'])
