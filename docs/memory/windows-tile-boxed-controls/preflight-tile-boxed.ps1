...
$record=[ordered]@{schema='tile-boxed-preflight-v1';dxdiag=$dx;dxdiagSource=$dxSource;cell_id=$CellId;cellId=$CellId;adapter='Intel(R) Graphics';count=16;cells=@('focused-one','focused-plus-background');roles=@('baseline','candidate');backgroundFps=1;utc=[DateTime]::UtcNow.ToString('o');powerState=$powerState;consoleSessionId=$consoleSid;vm=($vm|Select-Object Name,@{n='State';e={[string]$_.State}},MemoryAssigned);quietServices=@($services|Select-Object Name,@{n='Status';e={[string]$_.Status}});drivers=$drivers;sessions=$sessions;processes=$processes;server=$serverRecord;processLasso=[ordered]@{running=(@($lasso).Count -gt 0);processes=$lasso};strict_binding='unchanged';performanceAcceptance=$false;stages=@(@{role='baseline';path=$baselineStage;binaryPresent=(Test-Path (Join-Path $baselineStage 'panel-play.exe'))},@{role='candidate';path=$candidateStage;binaryPresent=(Test-Path (Join-Path $candidateStage 'panel-play.exe'));stageRequiredBeforeLaunch=$true})}
