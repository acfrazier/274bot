...
fn profile_args_are_exactly_what_the_native_executables_accept() {
    ...
    let (options, rest) = host_play::parse_profile_args(args.iter().map(String::as_str)).unwrap();
    assert!(
        rest.is_empty(),
...
fn test_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("274bot-suite-child-{tag}-{}", std::process::id()));
