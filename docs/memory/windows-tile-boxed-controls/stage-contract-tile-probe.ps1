param(
    ...
    [Parameter(Mandatory=$true)][ValidateSet('focused-one','focused-plus-background')][string]$Mode,
...
[ordered]@{schema='tile-boxed-contract-stage-v1';cell_id=$CellId;contract_id=$contractId;source=$source;destination=$destination;sourceSha256=$sourceSha;stagedSha256=$stagedSha;privileged=$true;performanceAcceptance=$false}|ConvertTo-Json -Depth 5
