# Observed fire lighting and host burn-tile selection
...
Complete the Firemaker and CookBot Fire option requirements with a Rust-owned
lightFire operation and bounded next-tile selection in the already posted
FIRE_PLOTS. Read exact start/light tick bounds from frozen Firemaking.ts; do
not invent timeout constants or clone findBurnLane's foreign ranker. Choose a
walkable selected-world tile inside the plot AABB, with no observed Fire loc
and outside the current refused set. Preserve plot ownership and nav lifecycle.

Lighting uses tinderbox on logs, then the actual XP or cannot-light game-chat
outcome; preserve lit/blocked/stalled return semantics and meaningful scene
change handling. toolRestockPlan needs ordinary tinderbox withdrawal through
the reviewed bank/provisioning services, not a general foreign Tools planner.
Excluded pickaxe/gatherer helpers stay deferred. Prove selected plot bounds,
blocked and unavailable terrain, observed fire/XP, log consumption and repeat
progress; queued item-on-item alone fails. Deliver 04-capabilities-fire.md and
evidence/fire-capabilities/.
