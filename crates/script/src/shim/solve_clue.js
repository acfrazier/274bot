...
// The posted shop interface the gate-toll trip reads. Both slots are posted
// only when the page carried them: an omitted `shop_open` is unobserved rather
// than a closed interface, and a row the page posted without a clickable slot,
// component or display name is still posted as the observation it is — the
// machine skips it rather than this file guessing at one.
...
// loc, the closed-handler `continue` / `answer` pair is never one either, and a
// `shop-button` click is its own landed op — never a loc, never a
// `kind: 'ops'` shopping list and never a `V2_OPS` author verb.
function enqueueClueVerb(step) {
    ...
    if (step.kind === 'shop-button') {
        // The open shop interface's own click: the posted stock row's identity
        // and one chunk of one. The landed `shop-button` op and nothing else —
        // its own explicit arm, never the loc fall-through, never a
        // `kind: "ops"` shopping list and never a `V2_OPS` author verb (the
        // shop module's own batch is not nested here).
        queue({
            op: 'shop-button',
            kind: step.shop,
            name: step.name,
...
export class SolveClue {
    ...
    async execute() {
        ...
        for (;;) {
            ...
            if (step.kind === 'supplies-needed' || step.kind === 'no-shop') {
                // The arrived dig with no `Spade` on the posted pack page, and
                // the gate-toll trip that could not observe the short's own
                // Trade, interface or stock row: named wait-classes and not
                // terminals, so the token lives and nothing is fetched. The
                // latch has the short, so the arm resumes and the same one
                // never shops twice.
