...
matched_evidence_adapter.py now recognizes one bounded native Windows observation shape. It requires non-empty platform/user identity, native preflight adapter/time/session, VM state, quiet-service records, driver identity records, session text, process record types, ProcessLasso running/process-list types, display card/driver identity, server identity, and explicit performance-acceptance booleans. It permits only the documented optional null display fields and process CommandLine values, and permits empty process lists. Required null, missing, empty, wrong-type, or malformed identity/control records remain invalid.
