...
r=obj(base+'CF2/result.json');prior=ref(r['ancestors'][0]);assert len(r['ancestors'])==1 and prior==ref(auth['parent'])
assert r['status']=='complete' and r['phase']=='CF2' and r['completed']==114
...
assert len(allchildren)==118 and len(aggregates)==59
budget=r['budget'];assert budget['children']==122 and budget['reserved_children']==budget['reserved_cpu']==budget['reserved_wall']==budget['unknown_cpu_charge']==0 and budget['stopped'] is False
assert budget['wall']<1800 and budget['cpu']<1500
checkpoint=ref(r['checkpoint']);ref(r['claim']);assert checkpoint['status']=='validated' and checkpoint['completed']==114
...
priorbudget=prior['budget'];original=r['prior_campaign_ledger'];assert original==prior['prior_campaign_ledger']
fullfresh={k:budget[k]-original[k] for k in ('wall','cpu')};projection={k:7.5*v for k,v in fullfresh.items()}
result=dict(qualified=True,scope='all59 feasibility raw/ledger audit only; no acceptance release or performance win',archive_sha256=args.archive_sha256,members=len(seen),completed=114,total_fresh_children=118,cumulative_budget=budget,original_budget=original,cf2_cost={k:budget[k]-priorbudget[k] for k in ('wall','cpu')},full_fresh_feasibility_cost=fullfresh,acceptance_projection=projection,projection_within_envelope=projection['wall']<=7200 and projection['cpu']<=6000,cpu_component_sampling_delta_s=component_delta,waited_minus_cf2_probe_cpu_s=budget['waited_children_cpu']-priorbudget['waited_children_cpu']-sum(c['child_cpu'] for c in allchildren if c['phase']=='CF2'),children=allchildren)
