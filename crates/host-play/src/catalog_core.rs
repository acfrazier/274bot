...
impl CoreCase {
    ...
    pub fn copies_retreat_spot(self) -> bool {
        matches!(self, Self::RetreatSpotV2)
    }
    ...
    pub fn retreat_spot_stop_reason(self) -> Option<&'static str> {
        match self {
            Self::RetreatSpotV2 => Some(RETREAT_SPOT_V2_STOP),
            _ => None,
        }
    }
...
pub struct Observation {
    ...
    /// Compact here/dest/kind retreat-spot witness. Empty unless the active Core
    /// case asked for retreat spot. Never a world copy.
    pub retreat: RetreatSpotObservation,
...
impl Observation {
    ...
    pub fn attach_retreat_spot(
        ...
        let here = self
            .tile
            .map(|(x, z, level)| LineOfSightTile { x, z, level });
        let receipt = paint.and_then(parse_retreat_spot_receipt_from_paint);
        let dest = receipt.as_ref().map(|row| row.dest);
        self.retreat = RetreatSpotObservation {
            available: scene.available,
            here,
            dest,
            receipt,
    ...
    pub fn attach_walk_spot(
        ...
        let here = self
            .tile
            .map(|(x, z, level)| LineOfSightTile { x, z, level });
        let receipt = paint.and_then(parse_walk_spot_receipt_from_paint);
        let dest = receipt.as_ref().map(|row| row.dest);
        self.walk = WalkSpotObservation {
            available: scene.available,
            here,
            dest,
            receipt,
    ...
    /// Compact host here plus the script paint walkOut/kind receipt. Never
    /// copies the collision grid or world. The curated box is here ± 2.
    pub fn attach_leave_lair(
        &mut self,
        snapshot: &GameSnapshot,
        paint: Option<&script::shim::ScriptPaint>,
    ) {
        ...
        let here = self
            .tile
            .map(|(x, z, level)| LineOfSightTile { x, z, level });
        let receipt = paint.and_then(parse_leave_lair_receipt_from_paint);
        let walk_out = receipt.as_ref().map(|row| row.walk_out);
        self.leave = LeaveLairObservation {
            available: scene.available,
            here,
            walk_out,
            radius: receipt.as_ref().map(|row| row.radius),
    ...
    /// Compact host here plus the script paint key-call receipt. Never copies
    /// the collision grid or world. The projected lair box is the File fixture,
    /// not the CELL box.
    pub fn attach_cell(
        &mut self,
        ...
        let here = self
            .tile
            .map(|(x, z, level)| LineOfSightTile { x, z, level });
        let receipt = paint.and_then(parse_cell_v2_receipt_from_paint);
        let door_walk = paint.is_some_and(cell_v2_paint_claims_door_walk);
        self.cell = CellV2Observation {
            available: scene.available,
            here,
...
fn parse_retreat_spot_receipt_from_paint(
    paint: &script::shim::ScriptPaint,
) -> Option<RetreatSpotScriptReceipt> {
    paint.lines.iter().find_map(|line| {
        line.strip_prefix(RETREAT_SPOT_RECEIPT_PREFIX)
            .and_then(|json| serde_json::from_str(json).ok())
    })
}

fn retreat_spot_kind_forbidden(kind: &str) -> bool {
...
fn retreat_spot_chebyshev(here: LineOfSightTile, dest: LineOfSightTile) -> i32 {
    (here.x - dest.x).abs().max((here.z - dest.z).abs())
}

fn retreat_spot_dest_ok(here: LineOfSightTile, dest: LineOfSightTile) -> bool {
    here.level == dest.level && (here.x != dest.x || here.z != dest.z) && {
        let d = retreat_spot_chebyshev(here, dest);
        d >= RETREAT_SPOT_MIN_CHEB && d <= RETREAT_SPOT_MAX_CHEB
    }
}
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct RetreatSpotObservation {
    pub available: bool,
    pub here: Option<LineOfSightTile>,
    pub dest: Option<LineOfSightTile>,
    pub receipt: Option<RetreatSpotScriptReceipt>,
}

impl Default for RetreatSpotObservation {
    fn default() -> Self {
        Self {
            available: false,
            here: None,
...
pub fn retreat_spot_baseline_ready(baseline: &Observation) -> bool {
    baseline.ingame
        && baseline.scene_state == 2
        && baseline.retreat.available
        && baseline.retreat.here.is_some()
}

fn retreat_spot_receipt_joined(now: &RetreatSpotObservation) -> bool {
    let (Some(here), Some(dest), Some(receipt)) = (now.here, now.dest, now.receipt.as_ref()) else {
        return false;
    };
    now.available
        && receipt.here == here
        && receipt.dest == dest
        && retreat_spot_dest_ok(here, dest)
        && retreat_spot_kind_ok(&receipt.kind)
        && !retreat_spot_kind_forbidden(&receipt.kind)
}

/// Post-Start witness: joined here/dest receipt, allowed kind, named stop.
#[derive(Debug, Clone, Default, Serialize)]
pub struct RetreatSpotDeliveryCycle {
    pub here: Option<LineOfSightTile>,
    pub dest: Option<LineOfSightTile>,
    pub kind: Option<String>,
    pub receipt: Option<RetreatSpotScriptReceipt>,
...
impl RetreatSpotDeliveryCycle {
    pub fn observe(&mut self, now: &Observation) {
        if self.receipt.is_some() {
            return;
        }
        if retreat_spot_receipt_joined(&now.retreat) {
            self.here = now.retreat.here;
            self.dest = now.retreat.dest;
            self.kind = now.retreat.receipt.as_ref().map(|row| row.kind.clone());
            self.receipt = now.retreat.receipt.clone();
        }
    }
    ...
    pub fn qualified(&self) -> bool {
        ...
        receipt.here == here
            ...
            && receipt.kind == kind
            && retreat_spot_dest_ok(here, dest)
            && retreat_spot_kind_ok(kind)
            && !retreat_spot_kind_forbidden(kind)
    }
}

fn parse_walk_spot_receipt_from_paint(
    paint: &script::shim::ScriptPaint,
) -> Option<WalkSpotScriptReceipt> {
    paint.lines.iter().find_map(|line| {
        line.strip_prefix(WALK_SPOT_RECEIPT_PREFIX)
            .and_then(|json| serde_json::from_str(json).ok())
    })
}

fn walk_spot_kind_forbidden(kind: &str) -> bool {
    kind.eq_ignore_ascii_case("walk-to")
...
fn walk_spot_dest_ok(here: LineOfSightTile, dest: LineOfSightTile) -> bool {
    here.level == dest.level && (here.x != dest.x || here.z != dest.z) && {
        let d = walk_spot_chebyshev(here, dest);
        d > 12 && d >= WALK_SPOT_MIN_CHEB && d <= WALK_SPOT_MAX_CHEB
    }
}
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct WalkSpotObservation {
    pub available: bool,
    pub here: Option<LineOfSightTile>,
    pub dest: Option<LineOfSightTile>,
    pub receipt: Option<WalkSpotScriptReceipt>,
}

impl Default for WalkSpotObservation {
    fn default() -> Self {
        Self {
            available: false,
            here: None,
...
pub fn walk_spot_baseline_ready(baseline: &Observation) -> bool {
    baseline.ingame
        && baseline.scene_state == 2
        && baseline.walk.available
        && baseline.walk.here.is_some()
}

fn walk_spot_receipt_joined(now: &WalkSpotObservation) -> bool {
    let (Some(here), Some(dest), Some(receipt)) = (now.here, now.dest, now.receipt.as_ref()) else {
        return false;
    };
    now.available
        && receipt.here == here
        && receipt.dest == dest
        && walk_spot_dest_ok(here, dest)
        && walk_spot_kind_ok(&receipt.kind)
        && !walk_spot_kind_forbidden(&receipt.kind)
}

/// Post-Start witness: joined here/dest receipt, walk radius 0, named stop.
#[derive(Debug, Clone, Default, Serialize)]
pub struct WalkSpotDeliveryCycle {
    pub here: Option<LineOfSightTile>,
    pub dest: Option<LineOfSightTile>,
    pub kind: Option<String>,
    pub receipt: Option<WalkSpotScriptReceipt>,
...
impl WalkSpotDeliveryCycle {
    pub fn observe(&mut self, now: &Observation) {
        if self.receipt.is_some() {
            return;
        }
        if walk_spot_receipt_joined(&now.walk) {
            self.here = now.walk.here;
            self.dest = now.walk.dest;
            self.kind = now.walk.receipt.as_ref().map(|row| row.kind.clone());
            self.receipt = now.walk.receipt.clone();
        }
    }
    ...
    pub fn qualified(&self) -> bool {
        ...
        receipt.here == here
            ...
            && receipt.kind == kind
            && walk_spot_dest_ok(here, dest)
            && walk_spot_kind_ok(kind)
            && !walk_spot_kind_forbidden(kind)
    }
}
...
const LEAVE_LAIR_BOX_PAD: i32 = 2;
const LEAVE_LAIR_EDGEVILLE_X: i32 = 3094;
const LEAVE_LAIR_EDGEVILLE_Z: i32 = 3493;
const LEAVE_LAIR_VARROCK_X: i32 = 3213;
const LEAVE_LAIR_VARROCK_Z: i32 = 3424;
const LEAVE_LAIR_KBD_X: i32 = 3017;
const LEAVE_LAIR_KBD_Z: i32 = 3849;
const LEAVE_LAIR_KBD_LOCS: [i32; 4] = [1765, 1766, 1816, 1817];

#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct LeaveLairBox {
    pub min_x: i32,
    pub max_x: i32,
    pub min_z: i32,
    pub max_z: i32,
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct LeaveLairObservation {
    pub available: bool,
    pub here: Option<LineOfSightTile>,
    pub walk_out: Option<LineOfSightTile>,
    pub radius: Option<i32>,
    pub area: Option<LeaveLairBox>,
    pub receipt: Option<LeaveLairScriptReceipt>,
}

impl Default for LeaveLairObservation {
    fn default() -> Self {
        Self {
            available: false,
            here: None,
...
fn leave_lair_value_forbidden(value: &Value) -> bool {
    match value {
        Value::Object(map) => {
            if map.get("key").and_then(Value::as_str) == Some("kbd-lair") {
                return true;
            }
            if map.get("inArea").and_then(Value::as_bool) == Some(false) {
                return true;
            }
            if map.get("left").and_then(Value::as_bool) == Some(true) {
                return true;
            }
            for key in ["locId", "id"] {
                if map
                    ...
                    .is_some_and(|n| LEAVE_LAIR_KBD_LOCS.contains(&(n as i32)))
                {
                    return true;
                }
            }
            for key in ["route", "exit", "gate", "outLever", "upLadder"] {
            ...
            map.values().any(leave_lair_value_forbidden)
        }
        Value::Array(rows) => rows.iter().any(leave_lair_value_forbidden),
        _ => false,
    }
}

/// Paint-row parser. A `!inArea` claim, KBD loc, or invented allow-flag is not a receipt.
pub fn parse_leave_lair_receipt_line(line: &str) -> Option<LeaveLairScriptReceipt> {
    let json = line.strip_prefix(LEAVE_LAIR_RECEIPT_PREFIX)?;
    if json.contains("!inArea") {
        return None;
    }
    let value: Value = serde_json::from_str(json).ok()?;
    if leave_lair_value_forbidden(&value) {
        return None;
    }
    serde_json::from_value(value).ok()
}

fn parse_leave_lair_receipt_from_paint(
    paint: &script::shim::ScriptPaint,
) -> Option<LeaveLairScriptReceipt> {
    paint
        .lines
        .iter()
        .find_map(|line| parse_leave_lair_receipt_line(line))
}

fn leave_lair_kind_forbidden(kind: &str) -> bool {
    kind.eq_ignore_ascii_case("walk")
        || kind.eq_ignore_ascii_case("teleport")
        || kind.eq_ignore_ascii_case("loc")
        || kind.eq_ignore_ascii_case("yield")
        || kind.eq_ignore_ascii_case("aborted")
        || kind.eq_ignore_ascii_case("kbd")
        || kind.eq_ignore_ascii_case("kbd-lair")
}
...
fn leave_lair_chebyshev(here: LineOfSightTile, walk_out: LineOfSightTile) -> i32 {
    (here.x - walk_out.x).abs().max((here.z - walk_out.z).abs())
}

fn leave_lair_box(here: LineOfSightTile) -> LeaveLairBox {
    LeaveLairBox {
        min_x: here.x - LEAVE_LAIR_BOX_PAD,
        max_x: here.x + LEAVE_LAIR_BOX_PAD,
        min_z: here.z - LEAVE_LAIR_BOX_PAD,
        max_z: here.z + LEAVE_LAIR_BOX_PAD,
        level: here.level,
    }
}

fn leave_lair_in_box(tile: LineOfSightTile, area: LeaveLairBox) -> bool {
    tile.level == area.level
        && tile.x >= area.min_x
        && tile.x <= area.max_x
        && tile.z >= area.min_z
...
fn leave_lair_forbidden_tile(tile: LineOfSightTile) -> bool {
    (tile.x == LEAVE_LAIR_EDGEVILLE_X && tile.z == LEAVE_LAIR_EDGEVILLE_Z)
        || (tile.x == LEAVE_LAIR_VARROCK_X && tile.z == LEAVE_LAIR_VARROCK_Z)
...
fn leave_lair_walk_out_ok(here: LineOfSightTile, walk_out: LineOfSightTile, area: LeaveLairBox) -> bool {
    here.level == walk_out.level
        ...
        && !leave_lair_in_box(walk_out, area)
        && !leave_lair_forbidden_tile(here)
        && !leave_lair_forbidden_tile(walk_out)
        && {
            let d = leave_lair_chebyshev(here, walk_out);
            d >= LEAVE_LAIR_MIN_CHEB && d <= LEAVE_LAIR_MAX_CHEB
        }
}

pub fn leave_lair_baseline_ready(baseline: &Observation) -> bool {
    baseline.ingame
        && baseline.scene_state == 2
        && baseline.leave.available
        && baseline.leave.here.is_some()
}
...
fn leave_lair_receipt_joined(now: &LeaveLairObservation) -> bool {
    ...
    now.available
        ...
        && now.radius == Some(receipt.radius)
        && receipt.discriminator.eq_ignore_ascii_case("gateless")
        && receipt.radius == LEAVE_LAIR_RADIUS
        && leave_lair_kind_ok(&receipt.kind)
        && !leave_lair_kind_forbidden(&receipt.kind)
        && leave_lair_walk_out_ok(here, walk_out, area)
}

/// Post-Start witness: joined here/walkOut receipt, gateless walk-near radius 3, named stop.
#[derive(Debug, Clone, Default, Serialize)]
pub struct LeaveLairDeliveryCycle {
    pub here: Option<LineOfSightTile>,
    pub walk_out: Option<LineOfSightTile>,
    pub kind: Option<String>,
    pub discriminator: Option<String>,
...
impl LeaveLairDeliveryCycle {
    pub fn observe(&mut self, now: &Observation) {
        if self.receipt.is_some() {
            return;
        }
        if leave_lair_receipt_joined(&now.leave) {
            self.here = now.leave.here;
            self.walk_out = now.leave.walk_out;
            self.kind = now.leave.receipt.as_ref().map(|row| row.kind.clone());
            self.discriminator = now
                .leave
                .receipt
                .as_ref()
                .map(|row| row.discriminator.clone());
            self.radius = now.leave.radius;
            self.area = now.leave.area;
            self.receipt = now.leave.receipt.clone();
        }
    }
    ...
    pub fn qualified(&self) -> bool {
        ...
        receipt.here == here
            ...
            && receipt.discriminator == discriminator
            && receipt.radius == radius
            && radius == LEAVE_LAIR_RADIUS
            && discriminator.eq_ignore_ascii_case("gateless")
            && leave_lair_kind_ok(kind)
            && !leave_lair_kind_forbidden(kind)
            && leave_lair_walk_out_ok(here, walk_out, area)
    }
}
...
/// Jail door rectangle. Proof fence, not a walk destination.
pub const CELL_V2_CELL: CellV2Box = CellV2Box {
    min_x: 2928,
    max_x: 2934,
    min_z: 9683,
    max_z: 9689,
...
/// File projection fixture. Mainland here must stay outside it.
pub const CELL_V2_LAIR: CellV2Box = CellV2Box {
    min_x: 40,
    max_x: 60,
    min_z: 40,
    max_z: 60,
...
/// Door stand the machine walks only after a jail key. A key-call must not target it.
pub const CELL_V2_DOOR: LineOfSightTile = LineOfSightTile {
    x: 2931,
    z: 9690,
    level: 0,
};
const CELL_V2_DUSTY_ID: i64 = 1590;
const CELL_V2_KBD_LOCS: [i32; 4] = [1765, 1766, 1816, 1817];

#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct CellV2Box {
    pub min_x: i32,
    pub max_x: i32,
    pub min_z: i32,
    pub max_z: i32,
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct CellV2Observation {
    pub available: bool,
    pub here: Option<LineOfSightTile>,
    pub cell: Option<CellV2Box>,
    pub boxes: Vec<CellV2Box>,
    pub door_walk: bool,
    pub receipt: Option<CellV2ScriptReceipt>,
}

impl Default for CellV2Observation {
    fn default() -> Self {
        Self {
            available: false,
            here: None,
...
fn cell_v2_number_is_dusty(n: i64) -> bool {
    n == CELL_V2_DUSTY_ID
}

fn cell_v2_is_door_tile(value: &Value) -> bool {
    let Value::Object(map) = value else {
        return false;
    };
    map.get("x").and_then(Value::as_i64) == Some(i64::from(CELL_V2_DOOR.x))
        && map.get("z").and_then(Value::as_i64) == Some(i64::from(CELL_V2_DOOR.z))
}

fn cell_v2_kind_forbidden(kind: &str) -> bool {
    matches!(
        kind.to_ascii_lowercase().as_str(),
        "walk"
            | "walk-near"
    ...
    match value {
        Value::Object(map) => {
            ...
            if map
                .get("kind")
                .and_then(Value::as_str)
                .is_some_and(cell_v2_kind_forbidden)
            {
                return true;
            }
            if map.get("dest").is_some_and(cell_v2_is_door_tile) {
                return true;
            }
            ...
            for key in ["locId", "id", "item", "itemId", "held"] {
                if map
                    .get(key)
                    .and_then(Value::as_i64)
                    .is_some_and(cell_v2_number_is_dusty)
                    || map
                        .get(key)
                        .and_then(Value::as_i64)
                        .is_some_and(|n| CELL_V2_KBD_LOCS.contains(&(n as i32)))
                {
                    return true;
                }
            }
            ...
            map.values().any(cell_v2_value_forbidden)
        }
        Value::Array(rows) => rows.iter().any(cell_v2_value_forbidden),
        Value::Number(n) => n.as_i64().is_some_and(cell_v2_number_is_dusty),
        Value::String(text) => {
            text.contains("1590")
                || text.eq_ignore_ascii_case("kbd-lair")
                || text.eq_ignore_ascii_case("entered")
        }
        _ => false,
    }
}

fn cell_v2_line_claims_door_walk(line: &str) -> bool {
    let door = line.contains("2931") && line.contains("9690");
    if !door {
        return false;
    }
    let lower = line.to_ascii_lowercase();
    lower.contains("walk") || lower.contains("dest")
}

/// Paint-row parser. A dusty-key claim, a door walk, or KBD is not a receipt.
pub fn parse_cell_v2_receipt_line(line: &str) -> Option<CellV2ScriptReceipt> {
    let json = line.strip_prefix(CELL_V2_RECEIPT_PREFIX)?;
    if json.contains("1590") || json.contains("kbd-lair") || cell_v2_line_claims_door_walk(json) {
        return None;
    }
    let value: Value = serde_json::from_str(json).ok()?;
    if cell_v2_value_forbidden(&value) {
        return None;
    }
    serde_json::from_value(value).ok()
}

fn parse_cell_v2_receipt_from_paint(
    paint: &script::shim::ScriptPaint,
) -> Option<CellV2ScriptReceipt> {
    paint
        .lines
        .iter()
        .find_map(|line| parse_cell_v2_receipt_line(line))
}
...
fn cell_v2_place_ok(here: LineOfSightTile, cell: CellV2Box, boxes: &[CellV2Box]) -> bool {
    cell == CELL_V2_CELL
        && boxes == [CELL_V2_LAIR]
        && !cell_v2_in_box(here, CELL_V2_CELL)
        && !cell_v2_in_box(here, CELL_V2_LAIR)
        && boxes.iter().all(|area| !cell_v2_in_box(here, *area))
}

pub fn cell_v2_baseline_ready(baseline: &Observation) -> bool {
    let Some(here) = baseline.cell.here else {
        return false;
    };
    baseline.ingame
        && baseline.scene_state == 2
        && baseline.cell.available
        && !baseline.cell.door_walk
        && !cell_v2_in_box(here, CELL_V2_CELL)
        && !cell_v2_in_box(here, CELL_V2_LAIR)
}

fn cell_v2_receipt_joined(now: &CellV2Observation) -> bool {
    ...
    now.available
        && !now.door_walk
        && receipt.here == here
        && receipt.discriminator.eq_ignore_ascii_case("key-call")
        && cell_v2_kind_ok(&receipt.kind)
        && !cell_v2_kind_forbidden(&receipt.kind)
        && cell_v2_place_ok(here, cell, &now.boxes)
}

/// Post-Start witness: joined key-call receipt and the named stop.
#[derive(Debug, Clone, Default, Serialize)]
pub struct CellV2DeliveryCycle {
    pub here: Option<LineOfSightTile>,
    pub kind: Option<String>,
    pub discriminator: Option<String>,
    pub cell: Option<CellV2Box>,
...
impl CellV2DeliveryCycle {
    pub fn observe(&mut self, now: &Observation) {
        if self.receipt.is_some() {
            return;
        }
        self.door_walk |= now.cell.door_walk;
        if cell_v2_receipt_joined(&now.cell) {
            self.here = now.cell.here;
            self.kind = now.cell.receipt.as_ref().map(|row| row.kind.clone());
            self.discriminator = now
                .cell
                .receipt
                .as_ref()
                .map(|row| row.discriminator.clone());
            self.cell = now.cell.cell;
            self.boxes = now.cell.boxes.clone();
            self.receipt = now.cell.receipt.clone();
        }
    }
    ...
    pub fn qualified(&self) -> bool {
        if self.stopped.is_none() || self.door_walk {
            return false;
        }
        let (Some(here), Some(kind), Some(discriminator), Some(cell), Some(receipt)) = (
            self.here,
            self.kind.as_deref(),
            self.discriminator.as_deref(),
            self.cell,
        ...
        receipt.here == here
            && receipt.kind == kind
            && receipt.discriminator == discriminator
            && discriminator.eq_ignore_ascii_case("key-call")
            && cell_v2_kind_ok(kind)
            && !cell_v2_kind_forbidden(kind)
            && cell_v2_place_ok(here, cell, &self.boxes)
    }
}
...
impl CoreWitness {
    ...
    pub fn observe(&mut self, observation: &Observation) {
        ...
        if matches!(self.case, CoreCase::RetreatSpotV2) {
            self.retreat_spot_cycle.observe(observation);
        }
    ...
    pub fn observe_script_lifecycle(&mut self, receipt: script::ScriptLifecycleReceipt) {
        match self.case {
            ...
            CoreCase::RetreatSpotV2 => {
                if let Some(expected) = self.case.retreat_spot_stop_reason() {
                    self.retreat_spot_cycle
                        .observe_script_lifecycle(receipt, expected);
                }
            }
            CoreCase::WalkSpotV2 => {
                if let Some(expected) = self.case.walk_spot_stop_reason() {
                    self.walk_spot_cycle
                        .observe_script_lifecycle(receipt, expected);
                }
            }
            CoreCase::EnterLairV2 => {
            ...
            CoreCase::LeaveLairV2 => {
                if let Some(expected) = self.case.leave_lair_stop_reason() {
                    self.leave_lair_cycle
                        .observe_script_lifecycle(receipt, expected);
                }
            }
            CoreCase::AcquireKeyV2 => {
            ...
            CoreCase::CellV2 => {
                if let Some(expected) = self.case.cell_stop_reason() {
                    self.cell_v2_cycle
                        .observe_script_lifecycle(receipt, expected);
                }
            }
...
impl CoreWatch {
    ...
    pub fn observe_snapshot_with_lifecycle(
        ...
        if matches!(
            ...
            let copies_retreat = match &*state {
                CoreWatchState::Ready { case, .. } | CoreWatchState::Failed { case, .. } => {
                    case.copies_retreat_spot()
                }
                CoreWatchState::Running { witness, .. } => witness.case.copies_retreat_spot(),
                CoreWatchState::Disabled | CoreWatchState::Qualified { .. } => false,
            };
            let copies_walk = match &*state {
                CoreWatchState::Ready { case, .. } | CoreWatchState::Failed { case, .. } => {
            ...
            if copies_retreat {
                observation.attach_retreat_spot(snapshot, paint);
            }
            if copies_walk {
    ...
    /// True only for the retreat-spot File card. Callers attach paint and
    /// compact here/dest only then.
    pub fn copies_retreat_spot(&self) -> bool {
        if !self.active.load(Ordering::Acquire) {
            return false;
    ...
    /// True only for the walk-spot File card. Callers attach paint and
    /// compact here/dest only then.
    pub fn copies_walk_spot(&self) -> bool {
        if !self.active.load(Ordering::Acquire) {
            return false;
    ...
    /// True only for the leave-lair File card. Callers attach paint and
    /// compact here/walkOut only then.
    pub fn copies_leave_lair(&self) -> bool {
        if !self.active.load(Ordering::Acquire) {
            return false;
        }
    ...
    /// True only for the cell File card. Callers attach paint and compact
    /// here/kind only then.
    pub fn copies_cell(&self) -> bool {
        if !self.active.load(Ordering::Acquire) {
            return false;
