...
- `build.rs` compares the byte contents of every frozen `crates/nav` and
  `crates/api` Rust/Cargo/build input and the client source/Cargo/build inputs
  before compiling. It rejects relevant dirty source and does not require the
  enclosing checkout HEAD to equal the frozen host commit.
- The executable reports both actual checkout refs and the deterministic frozen
  source-manifest digest (`ae06b3fa77a95d421a4c8f7e14fc862fce0103dec57ad098a6aaf65361db6608`
...
input/executable/source hashes, unique output directories, fixed CPU 30 s, wall
60 s, RSS 384 MiB, AS 512 MiB, and 1 MiB stdout/stderr budgets. REAL runs fail
...
- Four-test suite: `python3 -m unittest discover -s docs/memory/nav-uniform-census -p 'test_nav_uniform.py' -v` — 4 passed.
