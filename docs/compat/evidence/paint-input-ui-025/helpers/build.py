...
jobs=[jobs[1],('tui','platform_proof',['build','-p','tui','--example','platform_proof','--message-format=json'],'paint-tui-')]
for mode,name,args,prefix in jobs:
 ...
 receipt=dict(mode=mode,command=command,started_at=start,elapsed_seconds=round(time.monotonic()-before,3),exit_code=code,isolated_build=True,cargo_target_dir=str(target),target_started_empty=False,cache_reuse="Completed exclusive root cache reused for the source identity and hashes recorded in this receipt; no empty-started claim")
