    ...
    fn named(bank: NamedBank) -> Self {
        let access = bank.definition.and_then(|definition| {
            if let Some(npc) = definition.npc {
                Some(Access::Npc(NpcAccessArgs {
                    name: npc.name.into(),
                    op: npc.op.into(),
                    choose: definition.choose.unwrap_or_default().into(),
                }))
            } else {
                definition.object.map(|object| {
                    Access::Object(AccessArgs {
                        name: object.name.into(),
                        op: object.op.into(),
                        open_first: definition.open_first.map(|open| Opener {
                            name: open.name.into(),
                            op: open.op.into(),
                        }),
                    })
...
pub(crate) fn read_dest(value: &Value) -> Option<Dest> {
    ...
    let access = match (value.get("npcAccess"), value.get("access")) {
        (Some(npc), _) if !npc.is_null() => {
            Some(Access::Npc(serde_json::from_value(npc.clone()).ok()?))
        }
        (_, Some(object)) if !object.is_null() => {
            Some(Access::Object(serde_json::from_value(object.clone()).ok()?))
        }
        _ => None,
    };
...
pub(crate) struct BankingOpenArgs {
    ...
    destination: Value,
    #[serde(default)]
    pub(crate) booth_name: Option<String>,
    #[serde(default)]
    pub(crate) booth_op: Option<String>,
    #[serde(default)]
    prefer_nearby: Option<bool>,
    #[serde(default)]
...
impl BankingOpen {
    ...
    pub(crate) fn new(args: BankingOpenArgs, log_hook: Option<usize>) -> Self {
        Self {
            phase: Phase::Choose,
            stand: read_tile(&args.stand),
            destination: read_dest(&args.destination),
            booth_name: args.booth_name.unwrap_or_else(|| "Bank booth".into()),
            booth_op: args.booth_op.unwrap_or_else(|| "Use-quickly".into()),
            prefer_nearby: args.prefer_nearby != Some(false),
            nearby_radius: args.nearby_radius.unwrap_or(14.),
            unsupported_obstacles: args.obstacles.iter().any(|name| !name.trim().is_empty()),
            log_hook,
        }
    }
    ...
    fn choose(&mut self, cx: &mut Cx<'_>) -> Option<Step<bool>> {
        ...
        let (here, scene_booth) = observed::with(|scene| {
            ...
            let here = view.here().map(|t| WorldTile {
                x: t.x,
                z: t.z,
                level: t.level,
            });
            let distance = view
                ...
                .filter(|row| {
                    row.name
                        .as_deref()
                        .is_some_and(|name| name.eq_ignore_ascii_case(&self.booth_name))
                })
                .filter(|row| {
                    row.actions
                        .iter()
                        .any(|op| !op.is_empty() && op.as_ref() != "hidden")
                })
                .map(|row| row.distance)
                .min();
            let nearest_booth = view
                .nearest_booth()
                .filter(|booth| {
                    booth
                        .name
                        ...
                        && booth
                            .op
                            .as_deref()
                            .is_some_and(|op| op.eq_ignore_ascii_case(&self.booth_op))
                })
                .and_then(|booth| {
                    here.filter(|here| here.level == booth.tile.level)
                        .map(|here| {
                            here.x
                                .abs_diff(booth.tile.x)
                                .max(here.z.abs_diff(booth.tile.z))
                                as i32
                        })
                });
        ...
        if self.prefer_nearby
            && scene_booth.is_some_and(|distance| f64::from(distance) <= self.nearby_radius)
        {
            self.phase = Phase::Open(Target::Access(None));
        } else if let Some(bank) = nearest.filter(|bank| {
            self.prefer_nearby
                && here.is_some_and(|here| {
                    within(here, bank.tile, self.nearby_radius)
                        && self
                            .stand
                            .is_none_or(|stand| !within(here, stand, self.nearby_radius))
                })
        }) {
            ...
            if self.unsupported_obstacles {
                return Some(Step::Fail(Thrown::new("not impl: Banking.open.obstacles")));
            }
            let known = bank_select::nearest_bank(stand)
                .filter(|bank| within(stand, bank.tile, self.nearby_radius));
        ...
        } else if let Some(select) = SelectBank::start(
            SelectArgs {
    ...
    pub(crate) fn run(&mut self, cx: &mut Cx<'_>) -> Step<bool> {
        loop {
            match std::mem::replace(&mut self.phase, Phase::Choose) {
                Phase::Choose => {
                    if let Some(step) = self.choose(cx) {
                        return step;
                    }
                }
                ...
                Phase::Walk(mut walk, target) => {
                    if walk.step(cx).is_none() {
                        self.phase = Phase::Walk(walk, target);
                        return Step::Wait;
                    }
                ...
                Phase::Open(target) => match target {
                    Target::Access(access) => {
                        self.phase = match access {
                            Some(Access::Npc(args)) => {
                                Phase::Npc(Box::new(NpcAccess::new(args, self.log_hook)))
                            }
                            Some(Access::Object(args)) => {
                                Phase::Object(Box::new(BankAccess::new(args, self.log_hook)))
                            }
                            None => Phase::Object(Box::new(BankAccess::new(
                                AccessArgs {
                                    name: std::mem::take(&mut self.booth_name),
                                    op: std::mem::take(&mut self.booth_op),
                                    open_first: None,
                                },
                                self.log_hook,
                            ))),
                    ...
                    Target::Preset(stand) => match BankOpen::at_stand(
                        stand,
                        std::mem::take(&mut self.booth_name),
                        std::mem::take(&mut self.booth_op),
                        cx,
                    ) {
                ...
                Phase::Object(mut open) => {
                    let step = open.run(cx);
                    self.phase = Phase::Object(open);
                    return step;
                }
                Phase::Npc(mut open) => {
                    let step = open.run(cx);
                    self.phase = Phase::Npc(open);
                    return step;
                }
                Phase::Booth(mut open) => {
                    let step = open.step(cx);
                    self.phase = Phase::Booth(open);
                    return step;
                }
                Phase::Ready => {
                    let bank = BankView::now();
                    if bank.open && bank.loaded {
...
impl Family for BankingOpen {
    ...
    fn begin(args: Self::Args, _cx: &mut Cx<'_>) -> Begin<Self> {
        Begin::Run(Self::new(args, Some(0)))
    }
