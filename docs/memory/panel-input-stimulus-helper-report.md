...
Before moving the cursor and before every pulse, the helper verifies process path, PID, start time, binary hash, expected session, same-session ownership, foreground HWND, non-minimized state, HWND owner PID, and unchanged physical window bounds. It checks that the supplied point is inside the supplied physical window rectangle; the point's Game Image membership remains a root screenshot proof, not a guessed default.
...
The fixed schedule defaults to one pulse every 1000 ms with an 80 ms hold and alternates Left/Right. Duration is the explicit number of requested pulse slots. Scheduling uses a monotonic `Stopwatch`; a slot delayed by a full cadence is marked deferred/missed and the helper stops rather than issuing catch-up bursts. Any key-down receives a release attempt in the per-pulse `finally`, and a second bounded release attempt is made by the outer `finally` if an exception occurs after a key was pressed. A missing or failed release makes the result non-success.
