...
fn scratch() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "274bot-route-inspect-brimhaven-v2-{}",
        std::process::id()
    ));
...
fn live_like_snapshot(tick: u64) -> SnapshotInput<'static> {
    SnapshotInput {
        tick,
        here: Some(PIER),
        ingame: true,
...
fn post_tick(iso: &LoadIsolate, tick: u64, facts: NativeFactsInput<'_>) {
    iso.post_snapshot(script::isolate_fb::encode_snapshot_with_native(
        &live_like_snapshot(tick),
        facts,
    ));
...
fn actual_example_begins_and_settles_on_published_native_snapshot() {
    ...
    post_tick(&iso, 1, NativeFactsInput::default());
