...
q.write_json(out / 'launch.json', receipt)
try:
    ...
    for name, command, cwd in matrix:
        ...
        steps.append(step)
        q.write_json(out / 'steps.json', steps)
        print('END', name, step['exit'], round(step['wall_s'], 3), flush=True)
    receipt['all_requested_passed'] = all(s['exit'] == 0 and not s['timed_out'] and s['cleanup']['group_absent'] for s in steps)
finally:
    ...
    for path, digest in expected.items():
        assert sha(path) == digest, path
    q.write_json(out / 'result.json', receipt)
