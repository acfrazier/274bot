...
Dispatch by **profile name**, using profile defaults without task model/provider overrides. Verify the configured defaults before first dispatch and the actual model used by each review. For tasks warranting independent review under the verification guidance below, implementation hands the same card to profile `reviewer` with `kanban_request_review`, then stops; do not use a model name as the assignee or create a duplicate routine review card. Corrective reviews for wrong-model runs and the separate final `branchreviewer` card are required where applicable. Poll actual runs through review completion; an assignee label or task creation is not proof that a worker ran. Details: `docs/execution.md` in the active campaign checkout.

Do not skip the final Grok pass for substantial campaigns. Repo hygiene (remotes, force-push, submodules) is **orch inline**, not subagent-driven.
...
**Verification (operator clarification, 2026-09-10):** this is a hobby project; preserve code quality and existing optimization work with checks proportional to risk. Small mechanical, documentation and support-tool changes do not automatically need new tests, a separate review or a standalone report. Prefer a focused diff/syntax check or relevant existing test. Tests should catch meaningful behavior regressions, not mirror the implementation. Keep appropriate tests and independent review for protocol, lifecycle, navigation, rendering ownership and memory-sensitive changes; group related work into coherent reviewable tasks. Run affected crate tests with required features and client integration tests separately when their behavior is affected. The final Grok whole-branch review remains required for substantial campaigns. Delegated implementers stay within their named task; the orchestrator may complete all authorized cross-crate work. Details: `docs/execution.md`.
