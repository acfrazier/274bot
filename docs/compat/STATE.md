# Compatibility integration state

Updated 2026-09-11. This checkout is the isolated first integration batch:
`codex/rs2b0t-stage-1`, based on main b2bd5023489ab2e0b6ba690f228217e8984ac91b.
The ongoing full campaign remains in `.worktrees/rs2b0t-multirevision` with its
own current STATE and authorized local plan. See `08-incremental-integration.md`
for this batch's exact scope, verification and publication state. Historical
reports in this checkout are evidence snapshots, not active worker directions.

This batch contains the reconciled 274/289 client, immutable process profile,
revision-aware host boundary and selected-world navigation foundation. It does
not claim complete rs2b0t compatibility, every catalog card, frontend/platform
qualification or any performance result. Later banking and catalog capabilities
remain in the campaign checkout, including native ownership corrections for
bank/cake/autocast JavaScript sequencing.

Current status: formatting and strict Clippy passed;2603 workspace tests and
1007 separate client tests passed. Two unchanged external-catalog failures were
reproduced on exact main and retained separately. All eight controlled274/289
boundary/world/Guardian cells passed after restoring three historical fixture
corrections; the original failed door run is retained. See validation-ready.json
for source/proof mapping. Whole-branch Grok4.6 review t_fd3a8b47 is running on
exact code940531c3f/clientaef3952d. Approval, merge and push remain pending.

After acceptance, root publishes the client to acfrazier/FR-client-bothost
r274-bh-modular, verifies a fresh recursive checkout, then merges this ordinary
history into host main and pushes acfrazier/274bot. No release tag, package or
announcement is part of this integration. Root preserves unrelated primary
checkout files and all active campaign work.
