...
- spell teleports need carried runes and the relevant magic/quest prerequisites. Magic skill id 6 level 25 is already present in family 1; the missing facts are rune IDs/counts and any spell quest requirement. Source examples are Law/Air/Fire `50/150/50` (`nav-tele-smoke.ts:49-54`), defaults `80/200/80` (`navLiveHarness.ts:540-542`), and rich probe `200/500/200` (`transport-heavy-routes.ts:188-218`); item IDs for those live seed names are MISSING except for the Rust edge example `(554,1)+(556,3)+(563,1)`;
- jewellery teleport rows need carried charged jewellery. Live stress seeds carried `ring_of_dueling_8` and `amulet_of_glory_4` (`nav-stress-live.ts:462,492`); the glory OD is Al Kharid `(3293,3174,0)`→Edgeville `(3087,3496,0)`. Current facts have item 1712 worn, not carried, while bank supply has item 1712 ×1; a successful glory edge therefore needs carried item 1712, not merely worn equipment;
...
3. `spell-varrock`: `(3222,3218,0)`→`(3213,3424,0)`, `option_bits=1`, `state_family=needs schema extension`, `radius=8`, `model=0`; source `nav-tele-smoke.ts:101-205`. Freeze magic `(skill id 6, level 25)`, carried Law/Air/Fire rune item IDs/counts (live counts `50/150/50` at `:49-54`; numeric IDs MISSING), and any quest requirement; assertion is teleport log plus arrival, not golden bytes.
4. `jewellery-glory-edge`: `(3293,3174,0)`→`(3087,3496,0)`, `option_bits=1`, `state_family=needs schema extension`, `radius=8`, `model=0`; source `nav-stress-live.ts:462,492` and OD in `nav-stress-live.ts`. Freeze carried charged `amulet_of_glory_4` (numeric item id MISSING; Rust glory edge requires 1712 carried), not current worn-only 1712; assertion is jewellery log plus arrival.
...
8. `bank-fetch-nearest`: freeze one source-provenanced OD that actually reaches a bank edge, `option_bits=4`, `state_family=needs schema extension`, `radius/model=MISSING until selector`; source contract `docs/memory/nav-tiled-differential/README.md:98-116` plus host orchestration `crates/nav/src/bank_fetch.rs`. Freeze bank supply and missing carried/worn item (current diagnostic supply is Coins `995×10`, item `1712×1`), `allow_bank_fetch`, and the exact diagnosis that forces `plan_bank_fetch`; assertion is `BankSession` plan/result. An ordinary walk to a bank is not BankSession proof.
