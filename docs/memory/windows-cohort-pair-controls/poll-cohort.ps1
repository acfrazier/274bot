...
$result=[ordered]@{utc=[DateTime]::UtcNow.ToString('o');task=$name;cell_id=$CellId;count=16;mode='focused-one';observe_s=600;warmup_s=120;cohort_tail_s=5;teardown_grace_s=60;requestedBackend='gpu';taskState=if($task){[string]$task.State}else{'Missing'};lastTaskResult=if($info){$info.LastTaskResult}else{$null};completionPresent=(Test-Path $completion);startup=$startup;performanceAcceptance=$false};$result|ConvertTo-Json -Depth 6
if($Final){
