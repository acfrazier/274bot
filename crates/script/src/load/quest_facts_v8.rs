//! Typed local V8 marshalling for quest query helpers.
...
pub(super) fn install(runtime: &mut Runtime) -> Result<(), String> {
    ...
    let name = v8::String::new(&mut scope, "__rs2b0t_quest_facts_v2")
        .ok_or_else(|| "quest name".to_string())?;
    let func =
        v8::Function::new(&mut scope, quest_callback).ok_or_else(|| "quest fn".to_string())?;
    global
        .set(&mut scope, name.into(), func.into())
        .ok_or_else(|| "quest set".to_string())?;
    Ok(())
}
...
fn run_quest<'s>(
    ...
    match op.as_str() {
        "questIdentity" => quest_identity_op(scope, args.get(1)),
        "questPrereqs" => quest_prereqs_op(scope, args.get(1)),
        _ => Err("invalid-args".into()),
    }
}

fn quest_identity_op<'s>(
    scope: &mut v8::HandleScope<'s>,
    ...
    let pin = if is_id {
        ...
        api::quest_facts::IdentityPin::Name(&text)
    };
    match api::quest_facts::quest_identity(data.quest_identity(), pin) {
        Ok(value) => {
...
/// Exactly one string pin. Neither, both, a non-object, or a non-string is invalid-args.
fn identity_pin(
    scope: &mut v8::HandleScope,
    input: v8::Local<v8::Value>,
    ...
    let id = field(scope, input, "id")?;
    let name_set = !name.is_undefined();
    let id_set = !id.is_undefined();
    if name_set == id_set {
        return Err("invalid-args".into());
    }
    let value = if id_set { id } else { name };
    if value.is_null() || !value.is_string() {
        return Err("invalid-args".into());
    }
    Ok((id_set, js_to_string(scope, value)?))
}

/// Seed id only. An extra name is ignored. A missing or non-string id is invalid-args.
fn required_id(scope: &mut v8::HandleScope, input: v8::Local<v8::Value>) -> Result<String, String> {
    if input.is_null() || input.is_undefined() || input.is_array() || !input.is_object() {
        return Err("invalid-args".into());
    }
    let id = field(scope, input, "id")?;
    if id.is_null() || id.is_undefined() || !id.is_string() {
        return Err("invalid-args".into());
    }
    js_to_string(scope, id)
}
