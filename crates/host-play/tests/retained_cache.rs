...
fn rebuilt_artifacts_bind_decoded_inputs_and_actual_baker_sources() {
    use nav::manifest::{hash_file, CacheManifest, NavManifest};
    ...
    for (revision, engine, snapshots) in [
        ...
        let manifest: NavManifest = serde_json::from_slice(
            &std::fs::read(nav::manifest::nav_manifest_path(&pack)).unwrap(),
        )
        .unwrap();
        let source = nav::bundle::source_digest(
            &engine.parent().unwrap().join("content"),
            &[&nav::bake::config_jag_for(revision, &cache_dir).unwrap()],
        )
        .unwrap();
        assert_eq!(manifest.source_sha256.as_deref(), Some(source.as_str()));
        let hash = hash_file(&pack).unwrap();
        manifest
            .verify_pack(revision, &cache, &hash, Some(&id))
            .unwrap();
        for (extension, expected) in [
            ("navflags", &manifest.flags_sha256),
            ("navreach", &manifest.reach_sha256),
            ("navcanlight", &manifest.canlight_sha256),
        ] {
            assert_eq!(
                Some(hash_file(&pack.with_extension(extension)).unwrap()),
                *expected
            );
        }
        let row = host_play::BundledNavIdentity {
            ...
            nav_sha256: hash.clone(),
            flags_sha256: manifest.flags_sha256.clone(),
        ...
        .is_bundled());
        assert!(manifest
            .verify_pack(revision, &cache, &hash, Some(&"00".repeat(32)))
            .is_err());
        if revision == 289 {
            ...
            let public_id =
                client::content_identity::compute_decoded_content_identity(289, &public, &public)
                    .unwrap()
                    .content_id_hex();
            ...
            manifest
                .verify_pack(289, &public_cache, &hash, Some(&public_id))
                .unwrap();
        }
    }
}
...
fn custom_endpoint_template_does_not_inherit_supported_server_facts() {
    ...
    for (revision, engine) in [
        ("274", "experiments/Server/engine"),
        ("289", "experiments/lostcity-289/engine"),
    ] {
        ...
        assert!(
            ...
            "custom {revision} template inherited built-in facts"
        );
    }
}
