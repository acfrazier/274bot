"""Verify MuleCrafter full cycles with the mapped native inventory reader."""
import hashlib
import json
import pathlib
import re
...
for path in sorted((E / 'catalog-harness/live').glob(f'*-{SHORT}.json')):
    ...
    row.update(revision=int(r['revision']), receipt=str(path.relative_to(ROOT)),
               log=str(log.relative_to(ROOT)), build_provenance='3536 exact source blobs and isolated binary verified.',
               scope='Selected core/options only; remaining branches and final integrated frontend acceptance separate.')
    assert r['exit_code'] == 0 and r['case']=='mule_crafter'
...
# The native Mule image and CUA presentation were read during this root turn.
native_row = read(E / f'catalog-headed/summary-native-mule-{SHORT}.json')
...
assert len(rows) == 4 and all(r['outcome'] == 'PASS' for r in rows)
...
(E / f'catalog-harness/mule-{SHORT}.json').write_text(json.dumps(summary, indent=2) + '\n')
...
cases = {'MuleCrafter': ['mule_crafter']}
for cell in matrix['rows']:
    ...
    if selected:
        detail = 'Isolated9cc: ' + ', '.join(r['case'] + '=' + r['outcome'] for r in selected)
        ...
        cell['proof_refs'] = list(dict.fromkeys(cell.get('proof_refs', []) + [r['receipt'] for r in selected]
                                              + [f'docs/compat/evidence/catalog-harness/mule-{SHORT}.json']))
