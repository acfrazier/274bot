...
env.update(LIVE="1", BOT_TARGET="local", ENGINE_DIR=spec["engine"],
           ...
           NAV_FLAGS=spec["nav_flags"], BOT_MEMORY_N="1", BOT_MEMORY_WORKLOAD=spec.get("workload", "active"),
           BOT_MEMORY_OUTPUT=str(out / "samples.jsonl"), BOT_MEMORY_WARMUP_S=str(spec.get("warmup_s", 30)),
           BOT_MEMORY_OBSERVE_S=str(spec.get("observe_s", 120)), BOT_MEMORY_DIAGNOSTICS="1")
if spec.get("sustain", True):
