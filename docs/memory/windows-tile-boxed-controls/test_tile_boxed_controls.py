...
class BoxedTileControls(unittest.TestCase):
    ...
    def test_generated_spec_uses_clean_argv_for_both_roles(self):
        common = {
            ...
            "mem": pathlib.Path("/fixture/docs/memory"), "diag": ["panel", "16", "active", "--warmup", "30", "--observe", "120", "--focused-one", "--render-profile", "--gpu-completion-profile", "--scheduling-profile", "--responsiveness-profile", "--responsiveness-fine", "--failure-capture"],
        ...
        self.assertNotIn('"BOT_RENDER_OWNER_CENSUS"', CONTROLLER.read_text())
