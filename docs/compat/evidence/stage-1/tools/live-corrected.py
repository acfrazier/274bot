...
root=Path.cwd();campaign=root.parent/'rs2b0t-multirevision';ev=root/'docs/compat/evidence/stage-1/fixture-corrections';ev.mkdir(exist_ok=True);target=campaign/'.superpowers/review-exports/t_a74e9684-combat-target'
...
checks=[]
for label,args in [('fmt',['cargo','fmt','--all','--','--check']),('clippy',['cargo','clippy','-p','host-play','--test','world_boundary_live','--features','memory-profile','--','-D','warnings'])]:
 with (ev/(label+'.log')).open('x') as f:code=subprocess.call(args,env=env,stdout=f,stderr=subprocess.STDOUT)
 checks.append(dict(name=label,command=args,exit_code=code));assert code==0
(ev/'focused-checks.json').write_text(json.dumps(dict(host_commit=host,client_commit=client,checks=checks),indent=2)+'\n')
...
for revision in ['274','289']:
 ...
 for case in (['nav_door','guardian_lamp'] if revision=='274' else ['boundary','nav_full','nav_door','guardian_lamp']):
