...
if(-not (Test-Path $run)){throw 'Run directory missing'}; $completion=Join-Path $run 'completion.json'; $completionPresent=Test-Path $completion
$receipt=$null; if($completionPresent){$receipt=Get-Content $completion -Raw|ConvertFrom-Json}
...
if($null -ne $receipt -and $receipt.run_dir){$rawSources += [string]$receipt.run_dir}
