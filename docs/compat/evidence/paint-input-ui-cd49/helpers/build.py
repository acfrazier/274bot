...
for mode,name,args,prefix in jobs:
 ...
 receipt=dict(mode=mode,command=command,started_at=start,elapsed_seconds=round(time.monotonic()-before,3),exit_code=code,isolated_build=True,cargo_target_dir=str(target),target_started_empty=False,cache_reuse="Completed exclusive root combat99 cache reused for exact cd49 source; no empty-started claim")
