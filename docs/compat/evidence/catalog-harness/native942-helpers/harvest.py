...
import json,hashlib
root=Path(__file__).resolve().parents[2];ev=root/'docs/compat/evidence';short='native942'
...
for p in sorted((ev/'catalog-harness/live').glob('*-'+short+'.json')):
 ...
 rows.append(row)
cases=['brimhaven_agility','ardy_cakes','bone_burier','alcher_defaults','bank_fletcher_shafts','bank_fletcher_headless']
