...
Complete Game.teleport for the enabled AIO Teleport, FireGiant and GreenDragon
calls using actual selected-world non-target spellbook buttons. It must press
the spell and observe the caller-required completion; do not translate it into
a navigation walk. Resolve the available teleport buttons and requirements from
selected cache metadata, distinguishing target spells. Preserve call arguments,
return shape and existing waits by inspecting the frozen API and real callers.

Show observed Magic XP and destination tile transition, including remaining
runes/level, absent control, stale session and refusal paths in composed tests.
A queued button is not an arrival. Keep rune planning from brief 27 and public
fixture restrictions; no new transport/navigation policy. Deliver
04-capabilities-teleport.md and evidence/teleport-capabilities/.
