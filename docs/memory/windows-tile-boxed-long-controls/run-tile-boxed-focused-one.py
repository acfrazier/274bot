...
role = os.environ.get("TILE_BOXED_LONG_BUILD_ROLE", "")
mode = os.environ.get("TILE_BOXED_LONG_MODE", "")
cell_id = os.environ.get("TILE_BOXED_LONG_CELL_ID", "")
...
root = pathlib.Path(os.environ.get("TILE_BOXED_LONG_HOST_ROOT", home / "274bot-workspaces/3118e96/host"))
...
out = home / ("274bot-runs/managed-tile-boxed-long-" + cell_id)
...
def diagnostic_argv(binary_path, manifest_path, build_role, run_mode):
    ...
    argv = ["panel", "16", "active", "--binary", str(binary_path), "--build-manifest", str(manifest_path), "--build-role", build_role, "--sustain", "--warmup", "30", "--observe", "600", "--focused-one", "--no-diagnostics", "--render-profile", "--gpu-completion-profile", "--scheduling-profile", "--responsiveness-profile", "--responsiveness-fine", "--failure-capture"]
    return argv
...
clean_environment()
preflight_cell_id = os.environ.get("TILE_BOXED_LONG_PREFLIGHT_CELL_ID", cell_id)
preflight = pathlib.Path(r"C:\ProgramData\274bot-Test\renderer-owner-census-9268890") / ("preflight-tile-boxed-long-" + preflight_cell_id + ".json")
...
spec = {"id": "native-panel-tile-boxed-long-" + cell_id, "index": 1, "cell_id": cell_id, "build_role": role_info["manifest_role"], "mode": mode, "kind": "diagnostic", "frontend": "panel", "count": 16, "requested_backend": "gpu", "requested_adapter": "Intel(R) Graphics", "configured_background_fps": None, "binary": str(binary), "build_manifest": str(manifest), "server_identity_path": str(server_id), "host_conditions_path": str(conditions), "nav_pack": os.environ["NAV_PACK"], "nav_flags": os.environ["NAV_FLAGS"], "catalog_path": str(catalog), "launcher_argv": [sys.executable, str(mem / "run_diagnostic.py"), *diag], "diagnostic_argv": diag, "game_server_pid": pid, "ambient_helpers": {"bootstrap": os.getppid()}, "sampler_interval_s": 0.5, "max_wall_s": 1500, "observe_s": 600, "warmup_s": 30, "teardown_grace_s": 60, "process_backend": "system", "cache_dir": str(server / "data/pack/client"), "unpack_root": str(home / ".274bot/unpack"), "performance_acceptance": False}
sp = out / "spec.json"
dump(sp, spec)
dump(out / "started.json", {"pid": os.getpid(), "start_identity": wps.sample_process(os.getpid())["start_identity"], "started_unix": time.time(), "role": role, "mode": mode, "cell_id": cell_id, "count": 16, "observe_s": 600, "warmup_s": 30, "teardown_grace_s": 60, "requested_backend": "gpu", "requested_adapter": "Intel(R) Graphics"})
...
dump(out / "completion.json", {"exit_code": rc, "ended_unix": time.time(), "role": role, "mode": mode, "cell_id": cell_id, "count": 16, "observe_s": 600, "warmup_s": 30, "teardown_grace_s": 60, "requested_backend": "gpu", "requested_adapter": "Intel(R) Graphics", "performance_acceptance": False})
sys.exit(rc)
