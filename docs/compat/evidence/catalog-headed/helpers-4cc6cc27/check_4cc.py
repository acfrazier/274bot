...
root=pathlib.Path(__file__).resolve().parents[2];ev=root/'docs/compat/evidence/catalog-headed';m=json.loads((ev/'source-4cc6cc27.json').read_text());source=pathlib.Path(m['source_root']);target=root/'.superpowers/catalog-headed/target-isolated-4cc6cc27'
...
for label,args in [('catalog',['test','-p','host-play','--features','memory-profile','--test','catalog_boundary_live']),('scenario',['test','-p','scenario','--lib']),('clippy-catalog',['clippy','-p','host-play','--features','memory-profile','--test','catalog_boundary_live','--','-D','warnings'])]:
 p=ev/('checks-4cc6cc27-'+label+'.log');assert not p.exists();before=time.monotonic()
...
(ev/'checks-4cc6cc27.json').write_text(json.dumps(out,indent=2)+'\n');gatep=ev/'review-gates-4cc6cc27.json';gate=json.loads(gatep.read_text());gate['root_fixture_checks_passed']=ok;gatep.write_text(json.dumps(gate,indent=2)+'\n');raise SystemExit(not ok)
