...
The generated contract requires the exact frozen archive SHA, original H/C identities, controller review `e707e2d`, nav-pack SHA, a native generated qualification with `linux_generated_qualified: true`, and a direct-owner binary manifest containing `memory-owner-capture` while excluding `snapshot-dedup`. It records `releaseable: false` and `native_qualified: false` because this preparation does not perform native or live admission.
