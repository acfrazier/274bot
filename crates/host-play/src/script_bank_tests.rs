...
impl Gate {
    ...
    pub(super) fn bank_search(&self) { self.searches.fetch_add(1, Ordering::Relaxed); }
    pub(super) fn single_route(&self) { self.single_routes.fetch_add(1, Ordering::Relaxed); }
...
fn bank_pick_ignores_unresolved_geometry_and_keeps_ties_in_catalog_order() {
    ...
    let routes = find_many_with_avoid_bounded(
        &surface.collision, &surface.graph, tile(0, 0), &targets,
    ...
    assert!(a.settled_at < b.settled_at, "the reversed catalog row below must disagree with heap settlement order");
    ...
    for (banks, expected, kind) in [
        ...
        (vec![unknown, bank("second reachable", 0, 8), bank("first reachable", 8, 0)], 1, PickKind::Reachable),
...
impl Driver for HeldDriver {
    fn set_menu(&mut self, _: i32, _: i32, _: i32, _: i32, _: i32) { panic!("held bank walk sent a menu action"); }
    fn do_action(&mut self, _: i32) -> bool { panic!("held bank walk dispatched an action"); }
    fn try_move(&mut self, _: i32, _: i32, _: i32, _: i32, _: bool, _: i32, _: i32, _: i32, _: i32, _: i32, _: i32) -> bool {
        panic!("held bank walk sent movement");
    }
    fn local_route(&self) -> Option<(i32, i32)> { panic!("held follow must not run"); }
    fn build_base(&self) -> (i32, i32) { panic!("held follow must not run"); }
    fn loc_typecode(&self, _: i32, _: i32) -> Option<i32> { panic!("held follow must not run"); }
    fn out(&mut self) -> &mut dyn api::prot::Out { panic!("held bank walk wrote a packet"); }
    fn login(&mut self, _: &str, _: &str, _: bool) -> bool { panic!("held bank walk attempted login"); }
}
...
fn bank_pick_walk_reuses_the_winning_route_and_hold_keeps_it_without_sends() {
    ...
    let gate = Controlled::new();
    assert!(queue_bank_walk(&navs, "test", &world, None, tile(1, 1), None));
    gate.0.wait(1);
    // Retransmitting the raw verb does not enqueue another ranking flood.
    assert!(queue_bank_walk(&navs, "test", &world, None, tile(1, 1), None));
    gate.0.release();
    gate.0.wait(4);
    let expected = find_with(&world.as_ref().unwrap().collision, &world.as_ref().unwrap().graph,
        tile(1, 1), tile(0, 14), FindOptions::default(), &WorldState::empty()).unwrap();
    {
        let all = navs.lock().unwrap();
        ...
        assert_eq!(gate.0.single_routes.load(Ordering::Relaxed), 0, "reuse the winner rather than flood again");
    }
    super::super::step_nav_bot(&mut HeldDriver, "test", Some((1, 1, 0)), &GameSnapshot::new(),
        &navs, &Arc::new(Mutex::new(vec![])), world.as_deref(), true, false,
        || panic!("a held slot must not enter the reach/follow path"));
    assert_eq!(navs.lock().unwrap()["test"].route.as_ref().unwrap().dest, expected.dest);
}
...
fn bank_pick_walk_timeout_discards_the_winner_and_attempts_only_the_air_fallback() {
    ...
    let all = navs.lock().unwrap();
    let bot = &all["test"];
    assert!(bot.route.is_none(), "the late reachable winner must not arm");
    assert!(bot.walk_outcome_failed);
    assert_eq!((bot.walk_outcome_x, bot.walk_outcome_z), (7, 1), "failure belongs to the air fallback");
    assert_eq!(gate.0.searches.load(Ordering::Relaxed), 1);
...
fn bank_pick_walk_reset_abort_and_newer_route_lease_reject_late_results() {
    ...
    for replacement in 0..3 {
        let navs = navs(vec![bank("bank", 8, 8)]);
        let gate = Controlled::new();
        assert!(queue_bank_walk(&navs, "test", &world, None, tile(0, 0), None));
        gate.0.wait(1);
        match replacement {
        ...
        let before = {
            let all = navs.lock().unwrap();
            let bot = &all["test"];
            (bot.requested_route, bot.walk_outcome_seq, bot.walk_outcome_request_id)
        };
        gate.0.release();
        gate.0.wait(4);
        let all = navs.lock().unwrap();
        let bot = &all["test"];
        assert!(bot.route.is_none());
        assert_eq!((bot.requested_route, bot.walk_outcome_seq, bot.walk_outcome_request_id), before);
    }
}
...
fn bank_pick_latest_pending_selection_preserves_an_armed_walk() {
    let world = Some(world(false));
    let navs = navs(vec![bank("bank", 8, 8)]);
    let route = find_with(&world.as_ref().unwrap().collision, &world.as_ref().unwrap().graph,
        tile(0, 0), tile(0, 3), FindOptions::default(), &WorldState::empty()).unwrap();
    {
        let mut all = navs.lock().unwrap();
        let bot = all.get_mut("test").unwrap();
    ...
    let gate = Controlled::new();
    queue_bank_pick(&navs, "test", &world, None, tile(0, 0), false, 1, BankPreferences::default(), None);
    gate.0.wait(1);
    for id in 2..=3 {
        queue_bank_pick(&navs, "test", &world, None, tile(0, 1), false, id, BankPreferences::default(), None);
    }
    gate.0.release();
    gate.0.wait(4);
    let all = navs.lock().unwrap();
    ...
    assert_eq!(gate.0.searches.load(Ordering::Relaxed), 2, "only active and latest pending selection run");
...
fn bank_pick_near_walk_skips_ranking_and_only_approaches_the_selected_stand() {
    ...
    assert_eq!(gate.0.single_routes.load(Ordering::Relaxed), 1);
    assert_eq!(navs.lock().unwrap()["test"].route.as_ref().unwrap().dest, tile(4, 4));
}
...
fn bank_pick_v2_explicit_opt_ins_control_the_returned_bank_over_settings_defaults() {
    ...
    let mage = NamedBank { definition: catalog.iter().find(|b| b.name == "Mage Arena"), ..bank("Mage Arena", 3091, 3958) };
    let zanaris = NamedBank { definition: catalog.iter().find(|b| b.name == "Zanaris"), ..bank("Zanaris", 3092, 3958) };
    for (defaults, options, expected) in [
        (false, "use_mage_bank: true, use_zanaris_bank: false", "Mage Arena"),
        (false, "use_mage_bank: false, use_zanaris_bank: true", "Zanaris"),
        (true, "use_mage_bank: false, use_zanaris_bank: false", "Public"),
        (true, "", "Mage Arena"),
    ] {
...
"#), script::LoadShape::NativeTick, vec![], None,
        ...
        iso.post_settings_bag(serde_json::json!({"useMageBank": defaults, "useZanarisBank": defaults}).as_object().unwrap());
        let post = |tick, selection| {
            iso.post_snapshot(super::super::with_script_snapshot_input(
                tick, Some((3091, 3958, 0)), true, None, None, None, None, None,
                false, false, false, 0, false, 0, false, 0, false, None,
                Default::default(), Default::default(), |snapshot, mut native| {
        ...
        super::super::dispatch_script_interact_cached(
            &mut HeldDriver, &GameSnapshot::new(), None, Some((3091, 3958, 0)),
            &navs, &Some(world(false)), Some(state), "test", iso.drain_interacts(), None, None,
        );
        let result = navs.lock().unwrap()["test"].bank_pick.posted;
        post(2, result);
        assert_eq!(iso.probe("globalThis.result.ok && globalThis.result.value.name").unwrap(), expected);
        assert!(navs.lock().unwrap()["test"].route.is_none(), "selection must not move");
        iso.join();
    }
}
...
fn bank_pick_stop_invalidates_pending_selection_and_walk_before_worker_publication() {
    let play = crate::Play::new(&crate::PlayOptions {
        host: "127.0.0.1".into(), port: 43594,
        cache_dir: concat!(env!("CARGO_MANIFEST_DIR"), "/../../target/bank-pick-empty-cache").into(),
        lowmem: true, mainland: false,
    });
    for walking in [false, true] {
        ...
        *play.navs.lock().unwrap() = std::mem::take(&mut *fixture.lock().unwrap());
        let gate = Controlled::new();
        if walking {
            assert!(queue_bank_walk(&play.navs, "test", &Some(world(false)), None, tile(0, 0), None));
        } else {
            queue_bank_pick(&play.navs, "test", &Some(world(false)), None, tile(0, 0),
                false, 1, BankPreferences::default(), None);
        }
        gate.0.wait(1);
        ...
        assert!(all["test"].route.is_none(), "Stop must prevent the bank worker from arming movement");
    }
}
...
fn bank_pick_spawn_failure_settles_selection_or_refuses_walk_without_arming() {
    for walking in [false, true] {
        ...
        if walking {
            assert!(!queue_bank_walk(&navs, "test", &Some(world(false)), None, tile(0, 0), None));
        } else {
            queue_bank_pick(&navs, "test", &Some(world(false)), None, tile(0, 0),
                false, 1, BankPreferences::default(), None);
        }
        let all = navs.lock().unwrap();
        let bot = &all["test"];
        ...
        assert!(bot.route.is_none());
        assert!(bot.bank_pick.worker.is_none() && bot.bank_pick.current.is_none() && bot.bank_pick.pending.is_none());
    }
}
