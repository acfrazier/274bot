...
fn tracked_source(
    ...
    for file in files.lines().filter(|f| {
        prefixes.iter().any(|p| f.starts_with(p))
            && (f.ends_with(".rs") || f.ends_with("Cargo.toml") || f.ends_with("build.rs"))
    }) {
...
fn main() {
    ...
    let c = String::from_utf8(git(&client, &["rev-parse", "HEAD"]).expect("client identity"))
        ...
        .trim()
        .to_owned();
    let host_files = tracked_source(&host, HOST_FROZEN, &["crates/nav/", "crates/api/"])
        .expect("host frozen source");
    let client_files = tracked_source(&client, CLIENT_FROZEN, &["src/", "Cargo.toml", "build.rs"])
    ...
    println!("cargo:rerun-if-changed=../../../crates/nav/src/pack.rs");
    println!("cargo:rerun-if-changed=../../../crates/nav/src/collision.rs");
    println!("cargo:rerun-if-changed=../../../vendor/fr-client-rust");
}
