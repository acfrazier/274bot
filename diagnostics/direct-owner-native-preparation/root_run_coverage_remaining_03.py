...
a = p.parse_args()
r = a.root.resolve()
out = r / 'root-coverage-native-03-remainder'
out.mkdir()
...
for key in ('RUSTFLAGS', 'CARGO_ENCODED_RUSTFLAGS', 'RUSTC_WRAPPER', 'RUSTC_WORKSPACE_WRAPPER'):
    env.pop(key, None)
steps = []
receipt = {'scope': 'Linux remaining 10 commands only; not a complete 19-command qualification or live/performance admission',
           'prior_failed_attempt': 'root-coverage-native-02', 'prior_failure_archive_sha256': '86a99fa43829eafeb286bb5f6ee2e2a830d29094d2d959d3acbfd00858549ee1',
...
try:
    matrix = (q.gap_matrix(source) + q.feature_off_matrix(source) + q.feature_on_matrix(source))[9:]
    ...
    for name, command, cwd in matrix:
        ...
        if step['timed_out'] or not step['cleanup']['group_absent']:
            break
    receipt['all_requested_passed'] = len(steps) == 10 and all(s['exit'] == 0 and not s['timed_out'] and s['cleanup']['group_absent'] for s in steps)
finally:
...
raise SystemExit(0 if receipt['all_requested_passed'] and len(steps) == 10 else 1)
