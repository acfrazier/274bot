...
target=root/'.superpowers/catalog-headed'/('target-isolated-'+short)
assert not target.exists(), 'Use a new build label/target; preserve earlier attempts'
env=dict(os.environ);env.update(CARGO_TARGET_DIR=str(target),GIT_COMMIT=manifest['host_commit'],GIT_DIRTY='0')
jobs=[('headless','catalog_boundary_live',['test','-p','host-play','--test','catalog_boundary_live','--features','memory-profile','--no-run','--message-format=json'],'catalog-boundary-'),('native','catalog_watch',['build','-p','panel','--example','catalog_watch','--message-format=json'],'catalog-watch-')]
for mode,name,args,prefix in jobs:
 ...
 receipt=dict(mode=mode,command=command,started_at=start,elapsed_seconds=round(time.monotonic()-before,3),exit_code=code,isolated_build=True,cargo_target_dir=str(target),target_started_empty=True)
 ...
 (folder/(('binary-headless-' if mode=='headless' else 'binary-')+short+'.json')).write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt),flush=True)
