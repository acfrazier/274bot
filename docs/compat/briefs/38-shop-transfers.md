...
Complete enabled Shop.open/buy/sell/close calls in Rust. Publish the actual
shop player pack as well as stock for Sell. Verify both selected interfaces
(including candidate player container 3823) against cache data. Reuse existing
Interactions shop item operations; do not send a generic if-button at a stock
component or answer an unobserved count dialog. Preserve 10/5/1 batching with
at most five user-event packets per tick, held-count settlement 3000 ms then
one tick, and actual foreign open/close bounds/attempts. No JS shopOpBatch copy.

Return observed quantities, handling stock depletion, insufficient coins,
partial progress, full inventory, changing shop/session, stale exact item rows
and Buy vs Sell container identities. Queueing clicks is not a successful
transfer. Shop.buyById has no enabled caller in the current audit and remains
unavailable unless verified required by a frozen enabled branch. Verify trade
existing boundaries for preservation; this task does not add a foreign Trade
controller. Deliver 04-capabilities-shop.md and evidence/shop-capabilities/.
