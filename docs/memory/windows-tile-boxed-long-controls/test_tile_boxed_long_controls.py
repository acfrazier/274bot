...
class LongBoxedTileControls(unittest.TestCase):
    def test_actual_argv_parser_and_callback_contract(self):
        ...
        self.assertEqual(rmc.rd.requested_backend(args), "gpu")
    ...
    def test_negative_duration_mode_backend_and_missing_server_configuration_fail(self):
        ...
        self.assertFalse(ns["server_configuration_complete"]({"configuration": {"world_json_sha256": "x"}}))
        argv_contract = next(n for n in contract.body if isinstance(n, ast.FunctionDef) and n.name == "long_argv_contract_complete")
        ns = {"rmc": rmc}
        exec(compile(ast.Module(body=[argv_contract], type_ignores=[]), "check-tile-probe-contract.py", "exec"), ns)
        self.assertTrue(ns["long_argv_contract_complete"](parsed))
        for field, value in (("observe", 120), ("warmup", 120), ("focused_one", False), ("cpu_fallback", True), ("nav_captures", True)):
