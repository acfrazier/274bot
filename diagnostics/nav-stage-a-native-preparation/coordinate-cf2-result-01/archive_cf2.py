...
files=sorted([p for base in (release,root/'root-coordinate-cf1-admission',root/'root-coordinate-cf2-admission') for p in base.rglob('*') if p.is_file()])
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
arc=root/'root-coordinate-cf2-evidence.tar.gz'
...
(root/'root-coordinate-cf2-evidence.manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
