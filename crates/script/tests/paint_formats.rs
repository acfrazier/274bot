...
//! (`String(n)` round-trips NaN and Infinity instead of collapsing them).
...
/// `fmtDuration(mins)`: 0 ms, sub-second, hours, days, NaN, Infinity.
...
/// `fmtXpHr(gained, mins)`: the `—` cutoff at 0.5 min, ties, negative and NaN rates.
...
/// `etaHours(remaining, xpPerHour)`: null for a spent or idle rate.
...
/// `levelRow(gain, mins)` as `label|fraction|perHour|toGo|eta`, and the set the
/// Jive `paintLevels` rows are built from.
