...
root = base / 'platform-isolated-caf6b809'
engine = base / 'server-289'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
m = json.loads((root / 'source-manifest.json').read_text())
b = json.loads((base / 'platform-build-caf6b809.json').read_text())
...
binary = base / 'platform_proof-linux-caf6b809'
assert sha(binary) == j['binary_sha256']
(root / 'bin').mkdir()
shutil.copy2(binary, root / 'bin/platform_proof-caf6b809')
shutil.copy2(base / 'platform-build-caf6b809.json', root / 'platform-build.json')
assert sha(root / 'bin/platform_proof-caf6b809') == j['binary_sha256']
