...
- On the Windows host, first run `validate-tile-boxed-ast.ps1 -ControlDirectory <controls>` and `preflight-tile-boxed.ps1 -CellId <fresh role-mode id>` as privileged host gates; then run `stage-contract-tile-probe.ps1 -BuildRole baseline|candidate -Mode focused-plus-background|focused-one -CellId <same fresh role-mode id>` as the privileged stage step; then run `run-contractcheck-tile-probe.ps1 -BuildRole baseline|candidate -Mode focused-plus-background|focused-one -CellId <same fresh role-mode id>` as BotTest Interactive Limited; finally run `prepare-tile-probe.ps1 -BuildRole baseline|candidate -Mode focused-plus-background|focused-one -CellId <same fresh role-mode id>` as the privileged host gate to consume that no-launch receipt. The contract uses a distinct `-contractcheck` cell namespace and never writes the launch cell output.
