...
$checkedServer=[ordered]@{ProcessId=[int]$server.ProcessId;ParentProcessId=[int]$server.ParentProcessId;Name=[string]$server.Name;SessionId=[int]$server.SessionId;CreationDate=[string]$server.CreationDate;CommandLine=$server.CommandLine}
$record=[ordered]@{adapter='Intel(R) Graphics';count=16;cells=@('focused-one','focused-plus-background');backgroundFps=1;utc=[DateTime]::UtcNow.ToString('o');consoleSessionId=$consoleSid;vm=($vm|Select-Object Name,@{n='State';e={[string]$_.State}},MemoryAssigned);quietServices=@($services|Select-Object Name,@{n='Status';e={[string]$_.Status}});drivers=$drivers;sessions=$sessions;processes=$processes;server=$checkedServer;processLasso=[ordered]@{running=(@($lasso).Count -gt 0);processes=$lasso;files=$lassoFiles};dxdiag=$dx;performanceAcceptance=$false}
