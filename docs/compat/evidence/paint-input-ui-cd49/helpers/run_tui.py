...
manifest = json.loads((root/'docs/compat/evidence/catalog-headed/source-paintcd49.json').read_text())
build = json.loads((root/'docs/compat/evidence/catalog-headed/binary-tui-paintcd49.json').read_text())
job = build
...
out=root/'docs/compat/evidence/paint-input-ui-cd49'/('tui'+revision+'-'+label)
...
cmd = [str(binary), '--live', 'script_paint_buttons_ui', '--profile', 'local-' + revision,
...
r = dict(command=cmd, pid=p.pid, started_at=started, environment=chosen,
         ...
         catalog_commit=catalog.name[7:], revision=revision, case='paint_buttons_ui', platform=sys.platform,
...
passed = 'Go bank pressed' in decoded and 'Resume pressed' in decoded
...
r.update(finished_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
         ...
         wall_timeout=timeout, runtime_error=bad, paint_callbacks_observed=passed, gameplay_acceptance=False,
