...
fn footprint_distance(tile: WorldTile, placement: &BankPlacement) -> i32 {
    let dx = (placement.x - tile.x)
        .max(tile.x - (placement.x + placement.width - 1))
        .max(0);
    let dz = (placement.z - tile.z)
        .max(tile.z - (placement.z + placement.length - 1))
        .max(0);
    dx.max(dz)
}
...
fn stand_for(
    ...
    let allowed = |tile: WorldTile| {
        walkable(tile)
            && placements
                .iter()
                .all(|placement| footprint_distance(tile, placement) != 0)
            && placements
                .iter()
                .any(|placement| footprint_distance(tile, placement) == 1)
    };
