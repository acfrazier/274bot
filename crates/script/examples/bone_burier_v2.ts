type BankStand = import('../host-js/index.d.ts').BankStand;
type ItemRow = import('../host-js/index.d.ts').ItemRow;
type NativeApi = import('../host-js/index.d.ts').NativeApi;

/**
 * Small native JS API v2 example. The TypeScript file is authoritative;
 * bone_burier_v2.js is produced with `deno bundle` for plain-JS users.
 */
...
let buried = 0;
let prayerXp = 0;
let initialPrayerXp: number | null = null;

function count(items: ItemRow[], name: string): number {
    return items
        .filter((item) => item.name?.toLowerCase() === name.toLowerCase() && !item.noted)
...
function observePending(api: NativeApi, bone: string, invCount: number): boolean {
...
export function tick(api: NativeApi): void {
    ...
    if (invCount > 0) {
        ...
        if (api.snapshot.bank_open) {
            ...
            api.request({ op: 'close' });
            return;
        }
        phase = 'burying';
        pending = { kind: 'bury', sent: api.tick, before: invCount, seq: 0 };
        api.request({ op: 'held', name: bone, action: 'Bury' });
        return;
    }
    ...
    if (api.snapshot.bank_open) {
        ...
        api.request({ op: 'withdraw-load', name: bone, bank_generation: api.snapshot.bank_generation });
        return;
    }
    ...
    api.request({
        ...
        name: stand.name,
        stand_op: stand.op,
        ...(stand.choose ? { choose: stand.choose } : {}),
    });
}
