...
for (const revision of [274, 289]) {
    ...
    if (!nurmof.aubury_travel.already_packed) throw new Error(`${revision}: aubury travel note missing`);
    ...
    if (flour.flour_barrel_tile.provenance !== 'derived' || flour.flour_barrel_tile.x !== 2735 || flour.flour_barrel_tile.z !== 3582) throw new Error(`${revision}: flour barrel tile ${JSON.stringify(flour.flour_barrel_tile)}`);
