...
def _fail(reason: str, *, cell_id: str = "") -> Dict[str, Any]:
    return {"schema": SCHEMA, "cellId": cell_id, "outcome": "incomplete",
...
def _required_binding(manifest: Dict[str, Any]) -> Optional[str]:
    ...
    for key in ("pid", "startUtc", "startIdentity", "sessionId", "binary", "binarySha256", "observedRect", "gameImagePoint"):
...
def run(manifest_path: Path, *, clock: Any = time, sampler: Callable[[int], Dict[str, Any]],
    ...
    if invalid:
        return _fail(invalid, cell_id=cell_id)
    target = manifest["target"]
    helper = manifest["helper"]
    output = manifest["output"]
    helper_path = Path(helper["path"])
    if not helper_path.is_file() or _sha(helper_path).lower() != HELPER_SHA256:
        return _fail("helper file missing or hash mismatch", cell_id=cell_id)
    if not Path(target["binary"]).is_file() or _sha(Path(target["binary"])).lower() != target["binarySha256"].lower():
        return _fail("target binary missing or hash mismatch", cell_id=cell_id)
    try:
        observed = sampler(int(target["pid"]))
    except Exception as exc:
        return _fail("target identity unavailable before launch: " + str(exc), cell_id=cell_id)
    if observed.get("start_identity") != target["startIdentity"]:
        return _fail("target start identity mismatch", cell_id=cell_id)
    now = clock.monotonic()
    ...
    if delay < 60 or delay > 90:
        return _fail("observe-start trigger window missed; no input spawned", cell_id=cell_id)
    ...
    args = [powershell, "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(helper_path),
            ...
            "-DurationSeconds", "120", "-CaptureEnabledVerified", "-SlotZeroFocusVerified"]
    started = clock.monotonic()
    try:
        process = popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    except Exception as exc:
        return _fail("helper launch failed before spawn: " + str(exc), cell_id=cell_id)
    spawn_count = 1
    ...
    for row in samples:
        ...
        if row["status"] == "available":
            process_summaries.setdefault(row["process"].removesuffix("-final"), {
                "pid": row["pid"], "start_identity": sample.get("start_identity"),
                "cpu_seconds": float(sample.get("user_s", 0)) + float(sample.get("system_s", 0)),
                "rss_bytes": sample.get("resident_bytes"), "samples": 0})
            process_summaries[row["process"].removesuffix("-final")]["samples"] += 1
    ...
    receipt = {"schema": SCHEMA, "cellId": cell_id,
               ...
               "resourceAccounting": {"status": "available" if process_summaries else "unavailable",
                                       "processes": list(process_summaries.values()),
               ...
               "helperExitCode": process.returncode, "stderr": stderr,
