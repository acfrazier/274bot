...
fn bank_access_stands_never_land_in_a_footprint_or_on_blocked_collision() {
    ...
    for bank in facts.banks() {
        let access = placements
            .iter()
            .find(|access| access.name == bank.name)
            .unwrap();
        assert!(
            bank.routable,
            "{} must resolve its supported access kind",
            bank.name
        );
    ...
    assert_eq!(
        facts.banks()[2].tile,
        CATALOG[2].tile,
        "valid preferred stand survives"
    );
}
...
fn a_bank_stand_can_have_a_wall_face_without_being_a_blocked_footprint() {
    use crate::{
        collision::{pack_walk, WorldCollision},
    ...
    assert!(
        world.named_bank_facts().is_none(),
        "an early read must not bind empty placements"
    );
    ...
    let bank = facts
        .banks()
        .iter()
        .find(|bank| bank.name == "Varrock West")
        .unwrap();
    assert!(bank.routable, "a wall face is not a blocked bank stand");
    assert_eq!(bank.tile, t(3185, 3440));
    let route =
        crate::router::find(&world.collision, &world.graph, t(3184, 3440), bank.tile).unwrap();
