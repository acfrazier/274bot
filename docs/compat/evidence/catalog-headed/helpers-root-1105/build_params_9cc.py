...
root=pathlib.Path(__file__).resolve().parents[2];short=sys.argv[1];folder=root/'docs/compat/evidence/catalog-headed';manifest=json.loads((folder/('source-params-'+short+'.json')).read_text());source=pathlib.Path(manifest['source_root'])
for name,h in manifest['files'].items():assert hashlib.sha256((source/name).read_bytes()).hexdigest()==h,name
target=root/'.superpowers/catalog-headed/target-isolated-28d97f38'
assert target.is_dir()
env=dict(os.environ);env.update(CARGO_TARGET_DIR=str(target),GIT_COMMIT=manifest['host_commit'],GIT_DIRTY='0')
jobs=[('params-tui','platform_proof',['build','-p','tui','--example','platform_proof','--message-format=json'],'params-tui-')]
for mode,name,args,prefix in jobs:
 ...
 receipt=dict(mode=mode,command=command,started_at=start,elapsed_seconds=round(time.monotonic()-before,3),exit_code=code,isolated_build=True,cargo_target_dir=str(target),target_started_empty=False,cache_reuse="Completed root28 compiler cache reused with new exact9cc source; no empty-started claim")
 ...
 (folder/(('binary-headless-' if mode=='headless' else 'binary-tui-' if mode=='tui' else 'binary-params-tui-')+short+'.json')).write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt),flush=True)
