...
- invokes PowerShell once through an argv list, with exactly 120 pulses, 1000 ms cadence, and 80 ms holds. Geometry and point are written to a validated per-run JSON launch spec and consumed by a static PowerShell `-Command` launcher as typed `[int[]]` values; this avoids PowerShell `-File` positional array truncation while retaining one helper invocation and no shell interpolation of manifest values;
...
`python3 -m unittest discover -s docs/memory/windows-cohort-stimulus-accounting -v`: 13 passed.
...
Tests use fake clock, sampler, and subprocess adapters. They cover wrong target identity, missing session/UI verification, missing session binding with retained envelope, wrong helper hash, no completed receipt, missed trigger/no spawn, exactly one spawn with typed-array launch-spec binding, bounded hung-helper cleanup, unavailable sampler coverage after spawn, missing CPU counters, helper failure, receipt mismatch, and receipt/event archiving.
