...
impl LiveState {
    ...
    fn frame(&mut self, client: &mut client::client::Client, hold: bool) {
        ...
        let key = format!("{}-{}-{}-{}-{}-{}", observation.tick,
            client.gens.stat, client.gens.inv, client.gens.chat, client.gens.iface, hold);
        if key != self.diagnostic_key {
            self.diagnostic_key = key;
            println!("{}", json!({"phase": "private-resource-diagnostic", "hold": hold,
                "started": self.start_count, "observation": observation,
                "inventory_size": self.snapshot.inventory_size(),
                "bank_component_id": self.snapshot.bank_component_id(),
                "bank_side": self.snapshot.bank_side().iter().map(|i| (i.def.id, i.count)).collect::<Vec<_>>(),
                "raw_woodcutting_xp": client.stat_xp[8],
                "raw_woodcutting_level": client.stat_effective_level[8]}));
        }
