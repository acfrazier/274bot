...
for label,args in [('count-dialog',['test','-p','script','--test','client_adapter_count_dialog','--','--test-threads=1']),('clippy-count-dialog',['clippy','-p','script','--test','client_adapter_count_dialog','--','-D','warnings']),('catalog',['test','-p','host-play','--features','memory-profile','--test','catalog_boundary_live']),('scenario',['test','-p','scenario','--lib']),('clippy-catalog',['clippy','-p','host-play','--features','memory-profile','--test','catalog_boundary_live','--','-D','warnings'])]:
 p=ev/('checks-78cc99d0-'+label+'.log');assert not p.exists();before=time.monotonic()
...
ok=len(rows)==5 and all(r['exit_code']==0 for r in rows);out=dict(host_commit=m['host_commit'],client_commit=m['client_commit'],source_files_verified=len(m['files']),checks=rows,all_passed=ok,isolated_build=True)
(ev/'checks-78cc99d0.json').write_text(json.dumps(out,indent=2)+'\n');gatep=ev/'review-gates-78cc99d0.json';gate=json.loads(gatep.read_text());gate['root_fixture_checks_passed']=ok;gatep.write_text(json.dumps(gate,indent=2)+'\n');raise SystemExit(not ok)
