...
import json,hashlib
root=Path(__file__).resolve().parents[2];ev=root/'docs/compat/evidence';short='resource2d589'
...
for p in sorted((ev/'catalog-harness/live').glob('*-'+short+'.json')):
 ...
 rows.append(row)
cases=['gnome_chop','gnome_fletch_short','gnome_fletch_long','herblore_secondaries','flax_aio_pick']
