"""Verify station and Ardougne full cycles while retaining failed cells."""
import hashlib
import json
import pathlib
import re
...
CASES = ('ardy_cakes','ardy_thiever','ardy_thiever_knight','cook_bot','cook_bot_lobster','smelter_bot','smelter_bot_steel','flax_spinner')
rows=[]
...
(E/f'catalog-harness/station-ardy-{SHORT}.json').write_text(json.dumps(summary,indent=2)+'\n')
...
matrix_path=ROOT/'docs/compat/support-matrix.json';matrix=read(matrix_path);cases={'ArdyCakes':['ardy_cakes'],'ArdyThiever':['ardy_thiever','ardy_thiever_knight'],'CookBot':['cook_bot','cook_bot_lobster'],'SmelterBot':['smelter_bot','smelter_bot_steel'],'FlaxSpinner':['flax_spinner']}
for cell in matrix['rows']:
 ...
 if selected:
  detail='Isolated912: '+', '.join(r['case']+'='+r['outcome'] for r in selected)+'. Passed cells include ordered work, bank deposit, closed return and further work. Failed cells retained; full options/frontends separate.'
  cell.update(status='PARTIAL',status_detail=detail,fixture_readiness=detail);cell['proof_refs']=list(dict.fromkeys(cell.get('proof_refs',[])+[r['receipt'] for r in selected]+[f'docs/compat/evidence/catalog-harness/station-ardy-{SHORT}.json']))
