...
Before every frame, and again after the pixel copy but before saving or accepting it, the helper checks the target process identity, start UTC, session, HWND ownership, foreground status, non-minimized state, and exact stable window rectangle. Any change aborts the burst; a changed target is not counted or receipted as a valid frame. Each successful frame gets a sequential PNG plus timestamped JSON receipt containing PID, start UTC, session, binary identity/hash, HWND, rectangle, capture start/end timestamps, PNG hash, and explicit non-inference fields. A `<label>.ready` marker is written only after frame 1 and therefore gives the root operator a point at which to click the already-visible Lumbridge control independently.
