param([Parameter(Mandatory=$true)][ValidateSet('baseline','candidate')][string]$BuildRole,[Parameter(Mandatory=$true)][ValidateSet('focused-one')][string]$Mode,[Parameter(Mandatory=$true)][ValidatePattern('^[A-Za-z0-9_-]+$')][string]$CellId)
...
$preflight='C:\ProgramData\274bot-Test\renderer-owner-census-9268890\preflight-tile-boxed-long-'+$CellId+'.json'
...
$out=Join-Path 'C:\Users\BotTest\274bot-runs' ('managed-tile-boxed-long-'+$CellId); if(Test-Path $out){throw 'Cell output exists; choose a new cell id'}
Copy-Item $runner (Join-Path $stage ("run-"+$CellId+'.py')) -Force
$launcher=Join-Path $stage ("launch-"+$CellId+'.ps1'); $log=Join-Path 'C:\Users\BotTest\274bot-runs' ('managed-tile-boxed-long-'+$CellId+'-launcher.log')
@"
`$ErrorActionPreference='Continue'; `$ProgressPreference='SilentlyContinue'
`$env:TILE_BOXED_LONG_BUILD_ROLE='$BuildRole'; `$env:TILE_BOXED_LONG_MODE='$Mode'; `$env:TILE_BOXED_LONG_CELL_ID='$CellId'
& 'C:\Program Files\Python314\python.exe' '$stage\run-$CellId.py' *> '$log'; exit `$LASTEXITCODE
"@|Set-Content $launcher -Encoding UTF8
$name='274bot-BotTest-tile-boxed-long-'+$CellId; if(Get-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue){throw 'Task exists; preserve prior output'}
...
[ordered]@{task=$name;cell_id=$CellId;build_role=$BuildRole;mode=$Mode;count=16;observe_s=600;warmup_s=30;teardown_grace_s=60;requestedBackend='gpu';adapter='Intel(R) Graphics';runnerSha256=(Get-FileHash $runner -Algorithm SHA256).Hash.ToLower();startedUtc=[DateTime]::UtcNow.ToString('o');performanceAcceptance=$false}|ConvertTo-Json
