...
/// Matches `enter_lair_v2.ts` `STOP_OK`.
pub(crate) const ENTER_LAIR_V2_STOP: &str = "enter lair qualification complete";
...
pub(crate) fn enter_lair_v2_scenario() -> Scenario {
    ...
    Scenario {
        ...
        settings: ScenarioSettings {
            ...
            deadline: ENTER_LAIR_DEADLINE,
            start_script: None,
            start_file: Some("enter_lair_v2.ts"),
            wait_script_stop: Some(ENTER_LAIR_V2_STOP),
            terminal_shot: Some("enter_lair_v2_ts"),
            nav: gold_script_nav(),
            ..Default::default()
        },
    }
...
mod tests {
    ...
    fn file_card_uses_mainland_seed_and_named_stop() {
        ...
        assert_eq!(s.settings.start_file, Some("enter_lair_v2.ts"));
        assert_eq!(s.settings.start_script, None);
        assert_eq!(s.settings.wait_script_stop, Some(ENTER_LAIR_V2_STOP));
        assert_eq!(s.settings.deadline, ENTER_LAIR_DEADLINE);
        assert!(s.seed.mainland);
        assert!(s.settings.require_mainland_base);
        assert_eq!(s.settings.terminal_shot, Some("enter_lair_v2_ts"));
    }
}
